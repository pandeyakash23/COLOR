...
def make_dataset(mask_percentage=10): 
    ...
    ohe_valid = np.load(f'./acp_gravy_onlymasktrain/data/x_{which_data}.npy', allow_pickle=True)
    classes_valid = np.argmax(ohe_valid, axis=2)
    output_valid = np.load(f'./acp_gravy_onlymasktrain/data/y_{which_data}.npy', allow_pickle=True)
    seq_len_valid = np.load(f'./acp_gravy_onlymasktrain/data/len_{which_data}.npy', allow_pickle=True)     
 
    if mask_percentage > 0 and which_data=='train':
...
def initalize():    
    init_lr = np.load('./acp_gravy_onlymasktrain/model/init_lr.npy', allow_pickle=True)
    # init_lr = init_lr[0]
...
def motif_identification():  
    ...
    with torch.no_grad():   
        ...
        np.save(f'./acp_gravy_onlymasktrain/model/importance_{which_data}', store_importance)
