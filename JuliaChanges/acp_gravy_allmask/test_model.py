...
def make_dataset_with_masking(mask_percentage=0):
    ...
    """
    # Load training data
    ohe_valid = np.load("./acp_gravy/data/x_test.npy", allow_pickle=True)
    classes_valid = np.argmax(ohe_valid, axis=2)
    output_valid = np.load('./acp_gravy/data/y_test.npy', allow_pickle=True)
    seq_len_valid = np.load('./acp_gravy/data/len_test.npy', allow_pickle=True) 
...
def test():
    test_loader, valid_size  = make_dataset_with_masking(10)
