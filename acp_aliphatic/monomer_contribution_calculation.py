...
def motif_identification(num_epochs, init_lr, max_m):  
    ...
    for sample, (i_x,i_classes, i_seq, i_actual) in enumerate(test_loader):
        ...
        while f_pointer < 80:
            ''' # 50 here means that we are using 
            only top 50 features in representation R to obtain monomer importance.
    ...
    #     top_num_token = int(ceil(i_x.size(1)*20/100))
    ...
    # print(np.argmax(modified_x[10,0:20,:], axis=-1))
    # # # print(modified_x.shape)
    # np.save('./data/masked_x', modified_x)
