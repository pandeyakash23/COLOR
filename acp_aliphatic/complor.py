...
class complor_network(nn.Module):
    def __init__(self, num_classes, d_model, d_out, max_m, rank):
        ...
        self.cnn1 = nn.Sequential( nn.Conv1d(self.d_model,32,1, stride=1), 
                                   nn.ReLU(),
        ...
        self.nn = nn.Sequential(
                                nn.Linear(int(self.d_model*self.d_out*self.max_m),8),
                                nn.ReLU(),
    ...
    def forward(self, x,  classes, seq_len):
        ...
        # heat_map = heat_map.reshape((heat_map.size(0),-1))
        # heat_map = torch.sum(heat_map, axis=-1).reshape((-1,1))
    ...
    def forward_feature_importance(self, x,  classes, seq_len, f, permute):
        ...
        heat_map = self.nn(heat_map)# heat_map = torch.cat((out_1, out_2, out_3,out_4), dim=3)      
