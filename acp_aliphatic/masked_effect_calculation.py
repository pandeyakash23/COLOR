import torch
import numpy as np
import math
from torch.utils.data import Dataset, DataLoader
from torch.autograd import Variable
from sklearn.model_selection import train_test_split
import torch.nn as nn
import matplotlib.pyplot as plt
from sklearn import preprocessing
import random
import matplotlib as mpl
import os
import gc
import pandas as pd
import csv
from numpy import *
from torch.utils.tensorboard import SummaryWriter
from datetime import date
import time
import builtins
from sklearn.metrics import balanced_accuracy_score, confusion_matrix,mean_absolute_error,r2_score
from complor import complor_network, dataset

import sys
sys.path.insert(0, './data/')
from generate_property import output_property

  
top_per = input('Top how much percentage unmasked?')  
top_per = int(top_per)

## Dataloader
batch_size = 256 
 
def masking_function(ohe, seq_len, importance):
    revised_x  = ohe
    num_ex = ohe.shape[0]
    for k in range(num_ex):
        l = int(seq_len[k])
        ex_token = np.argsort(importance[k,0:l], axis=-1)
        ex_token = ex_token[::-1]
        top_num_token = int(ceil(l*top_per/100))
        sample_imp = tuple(ex_token[top_num_token:].tolist())
        revised_x[k,sample_imp,:] = 0
        revised_x[k,sample_imp,-1] = 1
    return revised_x  

def make_dataset(): 
    # ohe_valid = np.load('./data/masked_x.npy', allow_pickle=True) 
    ohe_valid = np.load('./data/x_test.npy', allow_pickle=True)
    output_valid = np.load('./data/y_test.npy', allow_pickle=True)
    seq_len_valid = np.load('./data/len_test.npy', allow_pickle=True) 
    token_imp = np.load('./data/test_importance.npy', allow_pickle=True)
    ohe_valid = masking_function(ohe_valid, seq_len_valid, token_imp)
    classes_valid = np.argmax(ohe_valid, axis=2)
    
    # print(classes_valid[10,0:20])
 
    test_dataset = dataset(ohe_valid,classes_valid,seq_len_valid,output_valid,ohe_valid.shape[0])

      
    test_loader = DataLoader(dataset=test_dataset,
                            batch_size=batch_size,
                            shuffle=False)   
    
    return  test_loader, ohe_valid.shape[0]


def test_after_masking():
    test_loader, valid_size  = make_dataset()
    with torch.no_grad():
        predicted_label = np.zeros((valid_size, 1))
        actual_label = np.zeros((valid_size, 1))
        count_valid = 0         
        for j, (i_x,i_classes, i_seq, i_actual) in enumerate(test_loader):
            i_x = i_x.to('cpu').numpy()
            i_seq = i_seq
            i_classes = i_classes
            i_actual = i_actual
            
            # forward pass    
            i_batch = len(i_x)
            
            i_classes =  i_classes.numpy().astype(int)
            iter_y_pred = output_property(i_x, i_seq)
            
            predicted_label[count_valid:count_valid+i_batch, 0] = iter_y_pred
            actual_label[count_valid:count_valid+i_batch, :] = i_actual
            count_valid += i_batch


        valid_r2 = r2_score(actual_label, predicted_label)
        mae = mean_absolute_error(actual_label, predicted_label)
        print(f'Test R2:{valid_r2}, MAE:{mae}')

        
if __name__=='__main__':
    cp_1 = time.time()
    max_m = int(4) 
    test_after_masking()
    cp_2 = time.time()
    print('Time Taken',cp_2-cp_1)
